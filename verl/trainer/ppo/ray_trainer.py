...
def compute_topk_metrics_for_wandb(log_probs: torch.Tensor) -> Tuple[float, float, float, float, float]:
    ...
    return avg_topk_mass.detach().item(), avg_gap_1_2.detach().item(), avg_gap_top5_pairs.detach().item(), avg_slope.detach().item() #avg_entropy.detach().item(),
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            data_source_lst.append(test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor.shape[0])) # (batch_size,)
            
            new_test_batch = test_batch[:self.config.data.train_batch_size * (len(test_batch) // self.config.data.train_batch_size)]
            new_test_batch.batch["response_mask"] = compute_response_mask(new_test_batch)
            test_log_prob = self.actor_rollout_wg.compute_log_prob(new_test_batch)
            entropys = test_log_prob.batch["entropys"]
            response_masks = new_test_batch.batch["response_mask"]
            loss_agg_mode = self.config.actor_rollout_ref.actor.loss_agg_mode
            entropy_agg += agg_loss(loss_mat=entropys, loss_mask=response_masks, loss_agg_mode=loss_agg_mode).detach().item()
            high_entropy_tokens += (entropys > 0.5).float().mean()   
            count += 1
            test_log_prob.batch.pop("entropys")
            
        print(f"entropy_agg: {entropy_agg}, high_entropy_tokens: {high_entropy_tokens}, count: {count}")
        metric_dict[f"val/entropy"] = entropy_agg / count
        metric_dict[f"val/high_entropy_tokens"]= high_entropy_tokens / count
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        ...
                        high_entropy_tokens = (entropys > 0.5).float().mean()   
                        old_log_prob_metrics = {"actor/entropy": entropy_agg.detach().item(), "actor/high_entropy_tokens": high_entropy_tokens.detach().item()}
                        ...
                        if "rollout_log_probs" in batch.batch.keys():
                            ...
                            avg_topk_mass, avg_gap_1_2, avg_gap_top5_pairs, avg_slope = compute_topk_metrics_for_wandb(batch.batch["first15_logprobs"])
                            metrics.update({
                            ...
                            avg_topk_mass, avg_gap_1_2, avg_gap_top5_pairs, avg_slope = compute_topk_metrics_for_wandb(batch.batch["last15_logprobs"])
                            metrics.update({
