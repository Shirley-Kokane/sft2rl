            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    ...
                    if self.config.calculate_log_probs:
                        target_num = min(target_len, len(response_ids)//2)
                        ...
                        if target_num < target_len or len(first15_logprobs) == 0:
                            first15_logprobs = extrapolate(first15_logprobs, target_len, self.cache_logprobs)
                            last15_logprobs = extrapolate(last15_logprobs, target_len, self.cache_logprobs)
